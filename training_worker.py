...
def run_worker_request(
    ...
    claim = RunClaim(request.job.artifacts.output_dir, request.run_id)
    try:
        claim.acquire()
        assert_manifest_launchable(request)
    except ActiveRunError as exc:
        claim.release()
        LOGGER.error("Standalone training worker refused launch: %s", exc)
        return 2
    try:
        manifest = AtomicRunManifest(
    ...
    try:
        ...
        if telemetry is not None or notifier is not None:
            progress(
                {
                    "event_type": "failure",
                    "level": "error",
